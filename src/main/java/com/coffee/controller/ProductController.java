...
public class ProductController {
    ...
    public ResponseEntity<?> insert(@RequestBody Product product){
        // @RequestBody : http를 사용하여 넘어온 데이터(body)를 자바 객체 형식으로 변환해줍니다.
        ...
        try{
            ...
            // 메소드 체이닝 : 점을 연속적으로 찍어서 메소드를 계속 호출하는것
            ...
            return ResponseEntity.ok(Map.of("message", "Product insert successfully", "image", imageFileName)); // (키 , 값 , 키 , 값)
    ...
    @GetMapping("/update/{id}") // 상품의 id 정보를 이용하여 해당 상품 Bean 객체를 반환해줍니다.
...
//        System.out.println(updatedProduct);
    ...
    private String saveProductImage(String base64image) {
        ...
        try { // FileOutputStream는 바이트 파일을 처리해주는 자바의 Steram 클래스
            FileOutputStream fos = new FileOutputStream(imageFile);
