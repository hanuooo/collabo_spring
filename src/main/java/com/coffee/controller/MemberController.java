...
@RestController // 해당 클래스는 웹 요청(from react)을 접수하여 처리해주는 컨트롤러 클래스 입니다.
@RequiredArgsConstructor // final 키워드 또는 @NotNull 필드가 들어 있는 식별자의 값을 외부에서 주입해줍니다 (인젝션)
...
public class MemberController {
    ...
    public ResponseEntity<?> signup(@Valid @RequestBody Member bean, BindingResult bindingResult){ // 회원 가입을 위한 컨트롤러 메소드
        // ResponseEntity : Http 응답 코드(숫자 형식)나, 적절한 메시지 등을 표현하기 위한 클래스입니다.
    ...
    public ResponseEntity<Map<String, Object>> login(@RequestBody Member bean){
        ...
        // member는 데이터 베이스에서 내가 조회한 회원 정보를 담고 있는 객체입니다.
        Member member = this.memberService.findByEmail(bean.getEmail());

        boolean isFound = false ; // 회원이 발견되면 true로 바뀜 (Fleg변수)

        if(member != null){ // 아이디가 존재함
            if(bean.getPassword().equals(member.getPassword())){ // 비밀번호까지 일치함
                isFound = true ;
            }
        }
        ...
        if(isFound == true){
            ...
            response.put("message", "ID 또는 PASSWORD가 잘못되었습니다.");
